...
const getrecipe=()=>{
  fetch(`${API}/cuisines/data?cuisines=indian`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
          ...
          <img className="flag" src="https://media.istockphoto.com/id/961226424/photo/indian-flag-waving-symbol-of-india.jpg?b=1&s=170667a&w=0&k=20&c=5SyGNguB6cxydsT94ma8Kxwq_gGWrqNYjvsQcX4T9Jo="/>
          <p className="flag1">Indian Cuisines</p>
        <div className="list">
            {recipelist.map((mv) => (<Indiandata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Indiandata({data}){
  return (
