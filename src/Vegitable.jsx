...
const getrecipe=()=>{
  fetch(`${API}/ingredients/data?main_content=vegitable`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://c4.wallpaperflare.com/wallpaper/147/545/208/nature-basket-apples-grapes-wallpaper-preview.jpg"/>
          <p className="flag1">Vegitable Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Vegitabledata key={mv.id} data={mv} />))}
        </div>
        </div>
    </div>
 )};

 function Vegitabledata({data}){
  return (
