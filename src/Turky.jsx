...
export function Turky () {
  const navigate=useNavigate()
  const [recipelist, setRecipelist] = useState([]);

const getrecipe=()=>{
  fetch(`${API}/cuisines/data?cuisines=turky`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
      <img className="flag" src="https://wallpapercave.com/wp/wp4172868.jpg"/>
          <p className="flag1">Turky Cuisines</p>
        <div className="list">
            {recipelist.map((mv) => (<Turkydata key={mv.id} data={mv} />))}
        </div>
        </div>
    </div>
 )};

 function Turkydata({data}){
  return (
