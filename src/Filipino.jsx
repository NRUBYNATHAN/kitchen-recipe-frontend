...
const getrecipe=()=>{
  fetch(`${API}/cuisines/data?cuisines=filipino`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
     <img className="flag" src="https://t4.ftcdn.net/jpg/00/99/31/57/360_F_99315795_7VRIsYnIElKZGs9bwOFL7HLG4BsXaOZX.jpg"/>
          <p className="flag1">Filipino Cuisines</p>
        <div className="list">
            {recipelist.map((mv) => (<Filipinodata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Filipinodata({data}){
  return (
