...
const getrecipe=()=>{
  fetch(`${API}/cuisines/data?cuisines=spanish`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
      <img className="flag" src="https://img.freepik.com/premium-vector/spanish-flag-spain-country-national-identity_8071-1617.jpg"/>
          <p className="flag1">Spanish Cuisines</p>
        <div className="list">
            {recipelist.map((mv) => (<Spanishdata key={mv.id} data={mv} />))}
        </div>
        </div>
    </div>
 )};

 function Spanishdata({data}){
  return (
