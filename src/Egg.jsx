...
const getrecipe=()=>{
  fetch(`${API}/ingredients/data?main_content=egg`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://c4.wallpaperflare.com/wallpaper/928/426/519/bacon-egg-tomato-salad-wallpaper-preview.jpg"/>
          <p className="flag1">Egg Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Eggdata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Eggdata({data}){
  return (
