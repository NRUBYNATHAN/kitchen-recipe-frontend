...
const getrecipe=()=>{
  fetch(`${API}/meals/data?meals=soup`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://wallpapers.com/images/featured/32bllokh9fz32gos.jpg"/>
          <p className="flag1">Soup Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Soupdata key={mv.id} data={mv} />))}
        </div>
        </div>
    </div>
 )};

 function Soupdata({data}){
  return (
