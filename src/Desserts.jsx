...
const getrecipe=()=>{
  fetch(`${API}/meals/data?meals=desserts`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://c4.wallpaperflare.com/wallpaper/503/768/942/food-chocolate-cake-cake-wallpaper-preview.jpg"/>
          <p className="flag1">Desserts Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Dessertsdata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Dessertsdata({data}){
  return (
