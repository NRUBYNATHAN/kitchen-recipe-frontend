...
const getrecipe=()=>{
  fetch(`${API}/ingredients/data?main_content=beef`)
      .then((val) => val.json())
  ...
  return (
   ...
   <img className="dish" src="https://wallpaperaccess.com/full/5094397.jpg"/>
          <p className="flag1">Beef Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Beefdata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Beefdata({data}){
  return (
