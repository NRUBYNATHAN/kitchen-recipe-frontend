...
const getrecipe=()=>{
  fetch(`${API}/ingredients/data?main_content=chicken`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://deckfamilyfarm.com/wordpress/images/2015/09/poultryHerbRoastedChicken.jpg"/>
          <p className="flag1">Chicken Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Chickendata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Chickendata({data}){
  return (
