...
const getrecipe=()=>{
  fetch(`${API}/cuisines/data?cuisines=chinese`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
      <img className="flag" src="https://t4.ftcdn.net/jpg/00/42/09/99/360_F_42099910_vtsWg2oz6uFviZQyQ3Z8iqepex9SfRuR.jpg"/>
          <p className="flag1">Chinese Cuisines</p>
        <div className="list">
            {recipelist.map((mv) => (<Chinesedata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};
