...
const getrecipe=()=>{
  fetch(`${API}/cuisines/data?cuisines=italian`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
      <img className="flag" src="https://thumbs.dreamstime.com/b/silk-italy-flag-italy-flag-over-silk-texture-167018764.jpg"/>
          <p className="flag1">Italian Cuisines</p>
        <div className="list">
            {recipelist.map((mv) => (<Italiandata key={mv.id} data={mv} />))}
        </div>
    </div>
    </div>
 )};

 function Italiandata({data}){
  return (
