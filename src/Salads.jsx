...
const getrecipe=()=>{
  fetch(`${API}/meals/data?meals=salads`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://c4.wallpaperflare.com/wallpaper/701/94/381/5bd35dee2f759-wallpaper-preview.jpg"/>
          <p className="flag1">Salads Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Saladsdata key={mv.id} data={mv} />))}
        </div>
        </div>
    </div>
 )};

 function Saladsdata({data}){
  return (
