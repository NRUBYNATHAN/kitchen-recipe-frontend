...
const getrecipe=()=>{
  fetch(`${API}/ingredients/data?main_content=Seafood`)
      .then((val) => val.json())
  ...
  return (
    ...
    <div>
   <img className="dish" src="https://img.freepik.com/premium-photo/set-seafood-dishes-fish-squid-octopus-black-stone-background-top-view-free-space-your-text_187166-40214.jpg?w=2000"/>
          <p className="flag1">Seafood Recipes</p>
        <div className="list">
            {recipelist.map((mv) => (<Seafooddata key={mv.id} data={mv} />))}
        </div>
        </div>
    </div>
 )};

 function Seafooddata({data}){
  return (
